...
- **`--scary` 没有默认值,这是有意的。** 问的是「这个作品会不会**突然**吓人?」
  「不会」是作者作的证,「没填」是我们不知道 —— 两者差着一条红线,所以这个字段不许留空。
  **但作品是你做的,就由你答,别为这一条去打断用户。** 你当然知道自己写没写这些:
    - 毫无预告的巨响、尖叫、音量突增(jump scare 音效)
    - 突然铺满屏幕的脸 / 血 / 闪白闪红
    - 没有任何前兆就开始的追逐、突然扑到镜头前的东西
    - 全黑或静音之后突然来一下

  **上面任何一条有** → `--scary yes`。**填 yes 不影响过审**,它只是给玩家一句提醒。
  **一条都没有** → `--scary no`。
  恐怖题材本身不禁 —— 禁的是**不打招呼地吓人**,所以「氛围阴森但没有突袭」照样填 `no`。

  **只有这两种情况才回头问用户:**
  (a) 作品不是你做的、你也没读过它的代码和素材;
  (b) 你读了但真判断不了(比如素材是你打不开的外部二进制)。
  这时如实说「这个我判断不了,你来定」——**别猜,更别默认填 no**。
