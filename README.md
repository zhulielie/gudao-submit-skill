...
`submit.py <文件夹>` 会把那个文件夹里的东西**原样压进去传上去**,
过审之后作品是**公开**的。所以:

- **别把项目根目录当作品目录** —— `.env`、密钥、笔记、导出的数据都会跟着走。
- 自检现在会**点名拦下**这类文件(`.env` / `*.pem` / `*.key` / `*.db` / `credentials*` …),
  投稿脚本也会拦;`--dry` 会**把要传的文件逐个列出来**,交之前看一眼。
- 拿不准,就把作品单独拷进一个干净的文件夹再交。
